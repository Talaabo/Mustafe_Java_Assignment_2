  ...
  public static void main(String[] args) {
    ...
    while(!gameOver) {
      if(playerTurn) {
        ...
        int enemyAttackType = randNum.nextInt(3) + 1;
        int enemyDamage = eWeapon.strike(enemyAttackType, enemy.getDexterity(), enemy.getStrength());

        // take the damage and pass it into the player's armour to reduce the damage
        if (enemyDamage > 0) { // if there is damage at all
            enemyDamage = pArmour.getFinalDamage(enemyDamage);
